...
{  
  ...
  public function getLightbox($descArea = false)
  {
    ...
    if ($descArea){
      ...
      $info.= '<table>';
      $info.= '<tr><td>'.$this->_getHelper('heidelpay')->__('IBAN').':</td><td><input type="text" name="heidelpay[gp_kto]" value=""></td></tr>';
      $info.= '<tr><td>'.$this->_getHelper('heidelpay')->__('BIC').':</td><td><input type="text" name="heidelpay[gp_blz]" value=""></td></tr>';
  ...
  public function validate()
  {
    ...
    if ($post['payment']['method'] == $this->_code) {
      if (empty($post['heidelpay']['gp_kto'])){
        Mage::throwException($this->_getHelper('heidelpay')->__('Please enter IBAN.'));
      }
      if (empty($post['heidelpay']['gp_blz'])){
        Mage::throwException($this->_getHelper('heidelpay')->__('Please enter BIC.'));
      }
